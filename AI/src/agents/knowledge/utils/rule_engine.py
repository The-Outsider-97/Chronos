...
class RuleEngine:
    ...
    def apply(self, knowledge_base: dict, verbose=False) -> dict:
        ...
        # Dynamic Selection: Filter low-performing rules if we have enough stats
        active_rules = self._select_active_rules()
        ...
        for rule in active_rules:
            ...
            try:
                ...
                # Update performance stats (time)
                self._update_rule_stats(rule["name"], exec_time=exec_time)
                ...
                self._update_rule_stats(rule["name"], success=False)

        return (inferred, traces) if verbose else inferred

    def _select_active_rules(self) -> List[Dict]:
        """Select rules based on dynamic weights and performance thresholds."""
        # If no stats yet, return all rules
        if not self.rule_stats:
            return self.rules
            
        selected = []
        for rule in self.rules:
            # Always include rules with high weights or insufficient data
            if rule["weight"] > 0.3 or self.rule_stats[rule["name"]]["calls"] < 10:
                selected.append(rule)
            else:
                # Probabilistic inclusion for exploration
                if time.time() % 10 < 1: # 10% chance to retry poor rules
                    selected.append(rule)
        return selected

    def _update_rule_stats(self, rule_name: str, exec_time: float = None, success: bool = None, cost: float = 0.0):
        """Update runtime statistics for a rule."""
        ...
        if cost > 0:
            ...
            stats["avg_cost"] = (stats["avg_cost"] * stats["calls"] + cost) / (stats["calls"] + 1)
            
        stats["last_updated"] = time.time()

    def record_feedback(self, rule_name: str, success: bool, cost: float = 0.0):
        """External feedback mechanism to tune rule weights."""
        self._update_rule_stats(rule_name, success=success, cost=cost)
        # Trigger weight adjustment periodically or on feedback
        if self.rule_stats[rule_name]["calls"] % 10 == 0:
            self.optimize_weights()

    def optimize_weights(self, decay: float = 0.95, learning_rate: float = 0.1):
        """Dynamically adjust heuristic weights based on observed performance."""
        logger.info("Optimizing heuristic weights based on performance metrics...")
        ...
        for rule in self.rules:
            ...
            success_rate = stats["successes"] / total_outcomes if total_outcomes > 0 else 0.5
            
            # 1. Time Penalty: Penalize rules significantly slower than threshold
            time_penalty = 0.0
            if avg_time > self.slow_rule_threshold:
                time_penalty = min(0.5, (avg_time - self.slow_rule_threshold) / self.slow_rule_threshold)
            
            # 2. Success Reward/Penalty
            # Compare against a baseline success rate (e.g., 0.5)
            performance_delta = success_rate - 0.5
            
            # 3. Cost Penalty
            cost_penalty = min(0.2, stats["avg_cost"])

            # Calculate new weight
            # Weight update rule: w_new = w_old * decay + lr * (performance - penalties)
            adjustment = learning_rate * (performance_delta - time_penalty - cost_penalty)
            new_weight = rule["weight"] * decay + adjustment
            
            # Clamp weights to reasonable range [0.1, 5.0]
            old_weight = rule["weight"]
            rule["weight"] = max(0.1, min(5.0, new_weight))
            
            if abs(old_weight - rule["weight"]) > 0.1:
                logger.debug(f"Adjusted weight for {name}: {old_weight:.2f} -> {rule['weight']:.2f} "
                             f"(SR: {success_rate:.2f}, Time: {avg_time:.3f}s)")
    ...
    def apply_by_sector(self, knowledge_base: dict, sector: str, verbose=False) -> dict:
        ...
        for rule in rules:
            # Dynamic Selection: Skip low-performing rules (90% chance)
            if self.rule_stats and (rule["weight"] <= 0.3 and self.rule_stats[rule["name"]]["calls"] >= 10):
                 if time.time() % 10 >= 1: 
                     continue
...
if __name__ == "__main__":
    ...
    # Simulate a rule
    def test_rule(kb):
        return {("Test", "is_a", "Success"): 0.9}
    
    engine.add_rule("TestRule", test_rule, weight=1.0)
    
    # Simulate usage (High Success)
    engine.rule_stats["TestRule"]["calls"] = 100
    engine.rule_stats["TestRule"]["total_time"] = 0.1 # Fast
    engine.rule_stats["TestRule"]["successes"] = 90 
    engine.rule_stats["TestRule"]["failures"] = 10
    
    print(f"Initial weight: {engine.rules[-1]['weight']}")
    engine.optimize_weights()
    print(f"Adjusted weight (High Success): {engine.rules[-1]['weight']}")
    
    # Simulate poor performance (Low Success)
    engine.rule_stats["TestRule"]["successes"] = 10
    engine.rule_stats["TestRule"]["failures"] = 90
    engine.optimize_weights()
    print(f"Adjusted weight (Low Success): {engine.rules[-1]['weight']}")
    
    # Simulate slow performance
    engine.rule_stats["TestRule"]["total_time"] = 100.0 # Slow (avg 1s)
    engine.optimize_weights()
    print(f"Adjusted weight (Slow): {engine.rules[-1]['weight']}")
